...
import { apiService, type MentorRecommendation, type MentorshipRequest, type MentorshipRelationship } from '../services/api';
...
export default function Mentorship() {
  ...
  const [requestMessage, setRequestMessage] = useState<{ [key: number]: string }>({});
  const [sendingRequest, setSendingRequest] = useState<number | null>(null);
  const [processingRequest, setProcessingRequest] = useState<number | null>(null);
  const [managingRelationship, setManagingRelationship] = useState<number | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      if (activeTab === 'recommendations') {
        const recommendationsData = await apiService.getMentorRecommendations(10);
  ...
  const handleRemoveRelationship = async (relationshipId: number) => {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cette relation de mentorat ?')) return;
  ...
  const handleBlockRelationship = async (relationshipId: number) => {
    if (!confirm('Êtes-vous sûr de vouloir bloquer cette relation de mentorat ?')) return;
          ...
          <div key={mentor.id} className="mentor-card">
            <div className="mentor-header">
              <div className="mentor-avatar">
                {mentor.avatar ? (
                  <img src={mentor.avatar} alt={`${mentor.prenom} ${mentor.nom}`} />
                ) : (
              ...
              <div className="mentor-info">
                <h3 className="mentor-name">{mentor.prenom} {mentor.nom}</h3>
            ...
            </div>
            {
              ...
              <div className="mentorship-areas">
                ...
                <div className="areas-list">
                  {mentor.mentorship_details.mentorship_areas.map((area, index) => (
              ...
              <div className="compatibility-scores">
                <div className="score-item">
                  <span>Expérience: {mentor.mentorship_details.experience_match}%</span>
                </div>
                <div className="score-item">
                  <span>Compétences: {mentor.mentorship_details.skill_alignment}%</span>
                </div>
                <div className="score-item">
                  <span>Compatibilité: {mentor.mentorship_details.compatibility_score}%</span>
                </div>
              </div>
            ...
            <div className="mentor-request-section">
              <textarea
                ...
                onChange={(e) => setRequestMessage(prev => ({ ...prev, [mentor.id]: e.target.value }))}
                className="request-textarea"
                rows={3}
              />
              <button
                onClick={() => handleSendRequest(mentor.id)}
                disabled={!requestMessage[mentor.id]?.trim() || sendingRequest === mentor.id}
                className="btn btn-primary"
              >
                {sendingRequest === mentor.id ? 'Envoi...' : 'Demander un mentorat'}
              </button>
            </div>
          </div>
        ))}
                ...
                <div key={request.id} className="request-card">
                  ...
                  {request.status === 'pending' && (
                    <div className="request-actions">
                      ...
                      >
                        {processingRequest === request.id ? 'Traitement...' : 'Accepter'}
                      </button>
                      <button
                ...
                <div key={relationship.id} className="relationship-card">
                  <div className="relationship-header">
                    <div className="person-avatar">
                      {relationship.mentor?.avatar ? (
                        <img src={relationship.mentor.avatar} alt={`${relationship.mentor.prenom} ${relationship.mentor.nom}`} />
                      ) : (
                        <div className="avatar-placeholder">
                          {relationship.mentor ? `${relationship.mentor.prenom[0]}${relationship.mentor.nom[0]}` : 'M'}
                        </div>
                      )}
                    </div>
                    <div className="person-info">
                      <h3 className="person-name">
                        {relationship.mentor ? `${relationship.mentor.prenom} ${relationship.mentor.nom}` : 'Mentor'}
                      </h3>
                      <p className="relationship-date">
                        Depuis le {new Date(relationship.date_added).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
                  ...
                  <div className="relationship-actions">
                    {relationship.status === 'active' ? (
                      ...
                      >
                        {managingRelationship === relationship.id ? 'Blocage...' : 'Bloquer'}
                      </button>
                    ) : (
                      <button
                        onClick={() => handleUnblockRelationship(relationship.id)}
                        disabled={managingRelationship === relationship.id}
                        className="btn btn-success btn-sm"
                      >
                        {managingRelationship === relationship.id ? 'Déblocage...' : 'Débloquer'}
                      </button>
                    )}
                    <button
                    ...
                    >
                      {managingRelationship === relationship.id ? 'Suppression...' : 'Supprimer'}
                    </button>
                  </div>
                </div>
              ))}
                  ...
                  <div className="relationship-header">
                    <div className="person-avatar">
                      {relationship.utilisateur?.avatar ? (
                        <img src={relationship.utilisateur.avatar} alt={`${relationship.utilisateur.prenom} ${relationship.utilisateur.nom}`} />
                      ) : (
                        <div className="avatar-placeholder">
                          {relationship.utilisateur ? `${relationship.utilisateur.prenom[0]}${relationship.utilisateur.nom[0]}` : 'U'}
                        </div>
                      )}
                    </div>
                    <div className="person-info">
                      <h3 className="person-name">
                        {relationship.utilisateur ? `${relationship.utilisateur.prenom} ${relationship.utilisateur.nom}` : 'Utilisateur'}
                      </h3>
                      <p className="relationship-date">
                        Depuis le {new Date(relationship.date_added).toLocaleDateString('fr-FR')}
                      </p>
  ...
  return (
    <div className="mentorship-container">
      <div className="mentorship-header">
        <h1 className="mentorship-title">Mentorat</h1>
