...
export default function StudyGroups() {
  ...
  const filteredGroups = groups.filter(group => {
    ...
    const matchesInterest = !selectedInterest || group.centre_interet === selectedInterest;
  ...
  return (
    <div className="study-groups-container">
      <div className="study-groups-header">
        <h1 className="study-groups-title">Groupes d'Étude</h1>
        <p className="study-groups-subtitle">
          Rejoignez des groupes d'étude et collaborez avec d'autres étudiants
        </p>
      </div>
      ...
      <div className="study-groups-stats">
        ...
        <div className="stat">
          <span className="stat-number">{filteredGroups.reduce((total, group) => total + group.nombre_membres, 0)}</span>
          ...
          <div key={group.id} className="study-group-card">
            <div className="group-header">
              ...
              {group.centre_interet && (
                <span className="group-interest-tag">{group.centre_interet}</span>
              )}
            </div>
            ...
            <div className="group-stats">
              <div className="group-members-count">
                👥 {group.nombre_membres} membre{group.nombre_membres > 1 ? 's' : ''}
              </div>
            </div>
            ...
            <div className="group-actions">
              ...
              >
                {expandedGroup === group.id ? 'Masquer les membres' : 'Voir les membres'}
              </button>
                    ...
                    {group.membres.map(member => (
                      <div key={member.id} className="member-card">
                        <div className="member-avatar">
                          {member.avatar ? (
                            <img src={member.avatar} alt={`${member.prenom} ${member.nom}`} />
                          ) : (
