          ...
          {dataArray.map((partner: any, index: number) => (
            <div key={index} className="skill-swap-card">
              <div className="partner-info">
                ...
                <p>
                  {partner.filiere} - {partner.niveau}iéme année
                </p>
                <div className="swap-score">
                  Score d'échange:{" "}
                  <span className="score">{partner.swap_score}</span>
                </div>
              </div>

              {partner.skills_they_offer &&
                partner.skills_they_offer.length > 0 && (
                  <div className="skills-section">
                    <h5>💡 Il peut vous aider avec:</h5>
                    <ul className="skills-list">
                      {partner.skills_they_offer.map(
                        (skill: any, idx: number) => (
                          <li key={idx}>
                            <strong>{skill.skill}</strong> (Niveau{" "}
                            {skill.their_level})
                            {skill.benefit && (
              ...
              {partner.skills_you_offer &&
                partner.skills_you_offer.length > 0 && (
                  <div className="skills-section">
                    <h5>🎯 Vous pouvez les aider avec:</h5>
                    <ul className="skills-list">
                      {partner.skills_you_offer.map(
                        (skill: any, idx: number) => (
                          <li key={idx}>
                            <strong>{skill.skill}</strong> (Votre niveau{" "}
                            {skill.your_level})
                            {skill.benefit && (
                              <span className="benefit">
                                {" "}
              ...
              {partner.mutual_benefits &&
                partner.mutual_benefits.length > 0 && (
                  <div className="mutual-benefits">
                    <h5>🤝 Avantages mutuels:</h5>
                    <ul>
                      {partner.mutual_benefits.map(
        ...
        <div className="generic-results">
          {dataArray.map((item: any, index: number) => (
            <div key={index} className="result-card">
              ...
              {item.centre_interet && (
                <span className="tag">{item.centre_interet}</span>
              )}
              {item.nombre_membres && (
                <span className="members">
                  👥 {item.nombre_membres} membres
                </span>
              )}
            </div>
          ))}
      ...
      return (
        <div className="single-response">
          <div className="response-card">
            ...
            {data.competence && (
              <p>
                <strong>Compétence:</strong> {data.competence}
              </p>
            )}
            {data.statut && (
              <p>
            ...
            {data.date_demande && (
              <p>
                <strong>Date:</strong>{" "}
                {new Date(data.date_demande).toLocaleDateString()}
              </p>
            )}
          </div>
        </div>
          ...
          <div className="chatbot-header">
            <div className="chatbot-title">
              ...
              <div>
                ...
                <p>
                  Votre assistant intelligent pour la collaboration étudiante
                </p>
              </div>
            </div>
